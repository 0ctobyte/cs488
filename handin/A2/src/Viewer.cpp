...
bool Viewer::clipLine(QVector4D& A, QVector4D& B) {
  ...
  // If any of the points are behind the W=0 plane, find their intersection with the W=0.00001 plane
  if(A.w() < 0.00001 && B.w() < 0.00001) {
    return true;
  } else if(A.w() < 0.00001) {
    float a = (A.w() - 0.00001) / (A.w() - B.w());
    A = A + a*(B-A);
  } else if(B.w() < 0.00001) {
    float a = (A.w() - 0.00001) / (A.w() - B.w());
    B = A + a*(B-A);
  ...
  for(uint32_t i = 0; i < 6; i++) {
    ...
    if((A.w() + P[i]) >= 0 && (B.w() + Q[i]) >= 0) continue;
    else if((A.w() + P[i]) < 0 && (B.w() + Q[i]) < 0) return true;
